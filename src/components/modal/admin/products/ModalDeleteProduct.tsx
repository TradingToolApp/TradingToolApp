...
import {useDeleteProduct} from "@/hooks/data/admin/useProducts";

const ModalDeleteProduct = ({modalData, open, handleClose, ...rests}: any) => {
    const deleteProduct = useDeleteProduct();
    const [loading, setLoading] = useState(false);
    const handleConfirmDelete = async () => {
        try {
            setLoading(true);
            const response = await deleteProduct.mutateAsync(modalData);
            if (response.success) {
                handleClose();
            }
            setLoading(false);
