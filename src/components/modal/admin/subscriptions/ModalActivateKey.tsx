...
import {useActivateKey} from "@/hooks/data/admin/useSubscriptions";

const ModalActivateKey = ({modalData, open, handleClose, ...rests}: any) => {
    const activateKey = useActivateKey();
    const [loading, setLoading] = useState(false);
    const handleConfirmDelete = async () => {
        try {
            setLoading(true);
            const response = await activateKey.mutateAsync(modalData.id);
            if (response.success) {
                handleClose();
            }
            setLoading(false);
...
export default ModalActivateKey;